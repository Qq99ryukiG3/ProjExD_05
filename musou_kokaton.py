...
class BossEnemy(pg.sprite.Sprite):
    ...
    Bossに関するクラス
    """
    imgs = pg.image.load(f"ex05/fig/alien1.png")
...
class Cure(pg.sprite.Sprite):
    ...
    def __init__(self, life: int):
        super().__init__()
        img = pg.image.load("ex_05/fig/item_cure.png")
        self.image = img
        self.rect = self.image.get_rect()
  ...
  # 200フレームに1回，敵機を出現させる
        ...
        for bossemy in pg.sprite.groupcollide(boss, beams, False, True).keys():
            ...
            if boss_life==0:
                ...
                bird.change_img(6, screen)  # 宇宙船喜びエフェクト
                screen.blit(gameclear,[280,220])
                score.update(screen)
                pg.display.update()
                time.sleep(2)
                return
        ...
        if len(pg.sprite.spritecollide(bird, nscs, True)) != 0:
            ...
            if bird.speed == 0: # スピードが0の時にゲームオーバー
                bird.change_img(8, screen) # 宇宙船悲しみエフェクト
                score.update(screen)
                meter.update(screen)
                pg.display.update()
                time.sleep(2)
        ...
        # shieldとatackの当たり判定
        for atack in atacks:
            if bird.shield_timer > 0 and bird.collides_with_shield(atack.rect):
                exps.add(Explosion(atack, 50))
                atack.kill()
